...
def write_files():
    ...
    # make directories
    os.system("mkdir trump >/dev/null 2>/dev/null")
    os.system("mkdir not_trump >/dev/null 2>/dev/null")
    ...
    i = 0
    for _ in range(1000):
        open("trump/" + str(i), "w").write(trump_tweets[0] + "\n")
        # train_data.write(trump_tweets[0] + "\n")
        trump_tweets.pop(0)
        # train_labels.write("1\n")

        open("not_trump/" + str(i), "w").write(non_trump_tweets[0] + "\n")
